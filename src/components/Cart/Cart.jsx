  ...
  confirm() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire("ORDER CONFIRMED", "Thank You For Choosing Us", "success");
    ...
    return (
      <React.Fragment>
        <div className="container cartHeader">
          <h1 className="CartName">
            {" "}
            {totalprice === 0
              ? "Cart"
              : `Cart (${product.map((ele) => ele.incart).length} item)`}
          </h1>
          <div>
            {totalprice === 0 ? (
              <div className=" text-center EmptCart">
                ...
                <button onClick={() => this.props.history.push("/MenProducts")}>
                  start shopping
                </button>
              </div>
            ) : (
          ...
          {product.map((ele) => (
            <div key={ele.id} className="row cartPage">
              <div className=" col-xs-12 col-sm-12 col-md-6 ">
                <div className="content">
                  ...
                  <div className="info">
                    ...
                    <p className="lead free d-none d-md-block">
                      Your order is eligible for Free Shipping with shop Express
                      or shop Primo. (Excludes large and bulky items)
                    </p>
                    ...
                    <div className="saved-Delete d-none d-md-flex">
                      <div
                        onClick={() => this.props.AddWish(ele)}
                        className="save"
                      >
                        <i
                          style={
                      ...
                      <div
                        onClick={() => this.props.onDelete(ele)}
                        className="Delete"
                      >
              ...
              <div className="  col-sm-4 col-md-2 d-none d-sm-block cartCount ">
                <div>
                  <button
                    onClick={() => this.props.onIncrement(ele)}
                    className="bttn"
                  >
                    +
                  </button>
                  <h3>{ele.count}</h3>
                  <button
                    onClick={() => this.props.onDecrement(ele)}
                    className="bttn"
                  >
                    -
                  </button>
